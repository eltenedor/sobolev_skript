...
\begin{proof}
  ...
  \item In the general case of Definition \ref{defn:boundarySpace}, let $\BB^i \coloneqq \BB_{s_i}(\Phi_i(x^i))$ such that $\Phi_i(U_i) \subset \BB^i$ (with $\Phi_i, \Psi_i$ from Lemma \ref{lem:flattening} with $\gamma = \gamma_i$ to flatten the boundary).
...
\begin{proof}
  \begin{enumerate}[i)]
    ...
    Then $\| u - \sum_{i = 0}^N f_i \|_{\WW^{1,p}(\Omega)} \leq \frac{\delta}{2}$ and $f \coloneqq \sum_{i = 0}^N f_i \in \WW^{1,p}(\Omega)$ with compact support in $\Omega$.
  ...
  \item It remains to find $w_m \in \WW^{1,p}(\BB^+)$ with $\supp w_m \Subset \BB^+$ and $w_m \to v$ in $\WW^{1,p}(\BB^+)$.
    We have
    \begin{equation}
    ...
    \end{equation}
    by ii) and Propositions \ref{prop:sobolevTransform} and \ref{prop:sobolevProperties}.
