...
\begin{thm}[Embeddings for $\WW^{k,p}(\Omega)$]
  ...
  \begin{enumerate}[a)]
    ...
    \item If $l < k - \frac{n}{p} \leq l + 1$ (i.e. $l = k - \Big[\frac{n}{p}\Big] - 1$) and, setting $\frac{n}{p} = 1$ if $p = \infty$,
      $$
...
\begin{proof}
  \begin{enumerate}[i)]
    ...
    \leq C_2 \, \sum_{|\alpha| \leq m - 1} \|\DD^\alpha u\|_{\WW^{1,r}}
    \leq C_3 \, \|u\|_{\WW^{m,r}(\Omega)}
    $$
    with $C_3$ depending on $n$, $r$, $q$, $m$, and $\Omega$.
    Hence, defining $p_0 \coloneqq p$ and $p_j \coloneqq p_{j - 1}^*$ for $j = 1,\dots,k - l$, we see that $\frac{1}{p_j} = \frac{1}{p_{j - 1}} - \frac{1}{n}$ and therefore $\frac{1}{p_j} = \frac{1}{p} - \frac{j}{n}$, $p_j = \frac{np}{n- jp}$ with $p_j > p_{j - 1}$.
...
\item \emph{Proof of a)}:
  ...
  $$
  \|u\|_{\WW^{l,p_{n - l}}(\Omega)} \leq C_4 \, \|u\|_{\WW^{k,p}(\Omega)}.
  $$
  As $\frac{1}{p_{n - l}} = \frac{1}{p} - \frac{k - l}{n}$ implies $k - \frac{n}{p} = l - \frac{n}{p_{k - l}}$ and hence $q \leq p_{n - l}$, Hölders inequality and the boundedness of $\Omega$ imply that $\WW^{l,p_{k - l}}(\Omega)$ is proved ($\|u\|_{\WW^{l,q}(\Omega)} \leq C_5\, \|u\|_{\WW^{l,p_{k - l}}(\Omega)}$
  \end{enumerate}
\end{proof}
