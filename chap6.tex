...
\item \emph{Proof of b)}:
  ...
  \item \emph{Proof of c)}: For $\frac{n}{p} \not\in\N_0$ we have $k - \frac{n}{p} < l + 1$ and \eqref{eq:sobolevIterationInequality} with $j = k - l - 2$ implies
    $$
...
\begin{proof}[Proof of Theorem \ref{thm:KolmogoroffRieszFrechet}]
  \begin{enumerate}[I)]
    \item Assume that $\FF \subset \Ell^p(\Omega)$ is precompact.
      ...
      $$
      As $\FF$ is precompact, there are $f \in \Ell^p(\Omega)$ and $(f_{m_k})_{k \in \N}$ such that $f_{m_k} \to f$ in $\Ell^p(\Omega)$.
    ...
    \begin{enumerate}[a)]
      \item We claim that for all $\delta > 0$ there exists $\varepsilon_\delta > 0$ such that
        ...
        \begin{align*}
          ...
          &= \int_{\BB_\varepsilon(0)} \eta_\varepsilon(z) \Big(\int_\Omega |\tau_{-z} \tilde f(x) - \tilde f (x) |^p \Big) \d z \\
      ...
      \item 
        ...
        Defining $\big(f_m^{(0)}\big)_{m \in \N} \coloneqq (f_m)_{m \in \N}$, we claim that for any $k \in \N$ there is a subsequence $\big(f_m^{(k)}\big)$ of the subsequence $\big(f_m^{(k - 1)}\big)_{m \in \N}$ such that
        \begin{equation}
          \label{eq:lpcauchy}
        ...
        \begin{equation}
          ...
          \| \big( g_{m_j} )_{\varepsilon_k} - \big( g_{m_i} \big)_{\varepsilon_k} \big) \|_{\Ell^p(\Omega)} \leq \frac{1}{3k} \quad\text{for all } i,j \geq N_k.
        \end{equation}
        Then
        \begin{align*}
  ...
  \begin{enumerate}[a)]
    \item Let $\varphi \in \CC_0^\infty(\R^n)$ and $h \in \R^n$.
      ...
      Then by \eqref{eq:embeddingpltn} $\FF$ is bounded in $\Ell^q(\Omega)$, i.e. i) in Thorem \ref{thm:KolmogoroffRieszFrechet} holds (for $q$), where $q \in [1,p^*)$.
        ...
        \intertext{which holds since $1 = \frac{1}{\frac{1}{q\theta}} + \frac{1}{\frac{p^*}{q(1 - \theta)}}$ and furthermore by the Hölder inequality}
      ...
      \item For $q \in [1,\infty)$ there is $\tilde p \in [1,n)$ such that $\tilde p^* > q$.
          As $\WW^{1,p}(\Omega)$ is continuously embedded into $\Ell^q(\Omega)$ by Proposition \ref{prop:embeddingspeqn} and any bounded sequence $(u_m)_{m \in \N}$ is also bounded in $\WW^{1,\tilde p}(\Omega)$ by Hölder's inequality, a) for $\tilde p$ shows that $(u_m)_{m \in \N}$ has a convergent subsequence in $\Ell^q(\Omega)$.
          This proves b).
