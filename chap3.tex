...
\begin{defn}
  \begin{enumerate}[a)]
    \item Let $k \in \N$, $p \in [1,\infty]$ and $\Omega \subset \R^n$ be open. 
      ...
      $$
      \WW^{k,p}(\Omega) \coloneqq \left\{u \in \Ell^p(\Omega) \colon \text{ weak derivative } D^\alpha \text{ exists with } \DD^\alpha \in \Ell^p, \forall 0 \leq |\alpha| \leq k \right\}
      $$
      with the norm
      $$
      \|u\|_{\WW^{k,p}(\Omega)} \coloneqq 
      \begin{cases}
        \left( \sum_{|\alpha| \leq k} \|\DD^\alpha\|_{\Ell^p(\Omega)}^p \right)^{\frac{1}{p}} &\text{ if } p \in [1,\infty), \\
          \sum_{|\alpha| \leq k} \|\DD^\alpha\|_{\Ell^\infty(\Omega)} &\text{ if } p = \infty.
      \end{cases}
      $$
      We further define
      $$
...
\begin{rem}
  \begin{enumerate}[a)]
    \item For $\alpha = (0,\dots,0)$ we set $\DD^\alpha u = \DD^0 u = 0$.
...
\begin{ex}
  ...
  Given $p \in [1,\infty)$, for which $a$ do we have $u \in \WW^{1,p}(\Omega)$?
    ...
    \begin{align*}
      u_{x_i}(x) &= -a|x|^{-a-1} \frac{x_i}{|x|} = -\frac{ax_i}{|x|^{a + 2}} \\
      |\nabla u(x)| &= \frac{|a|}{|a|^{a + 1}}.
    \end{align*}
...
\begin{prop}
  ...
  \begin{enumerate}[a)]
    ...
    \item If $\xi \in \CC_0^\infty(\Omega)$, then $\xi u \in \WW^{k,p}(\Omega)$ and Leibniz's formula
      $$
      \DD^\alpha(\xi u) = \sum_{\beta \leq \alpha} {\alpha \choose \beta} \DD^\beta\xi \DD^{\alpha - \beta} u
      $$
      holds with 
      $${{\alpha} \choose {\beta}} = \frac{\alpha!}{(\alpha -\beta)! \beta!}, \quad \alpha! = \prod_{i = 1}^n \alpha_i!$$
      and
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\overset{\mathrm{(IA)}}{=} (-1)^{|\beta|} \int_\Omega \sum_{\sigma \leq \beta} {\beta \choose \sigma} \DD^\sigma \xi\, \DD^{\beta - \sigma} u\, \DD^\gamma \varphi \d x \\
    &\overset{(3.3)}{=} (-1)^{|\beta|+|\gamma|} \int_\Omega \sum_{\sigma \leq \beta} {\beta \choose \sigma} \DD^\gamma \left( \DD^\sigma \xi \, \DD^{\beta - \sigma} u \right) \varphi \d x \\
    &= (-1)^{|\alpha|} \int_\Omega \sum_{\sigma \leq \beta} {\beta \choose \sigma} \left[ \DD^{\sigma + \gamma} \xi\, \DD^{\beta - \sigma} u + \DD^\sigma \xi \,\DD^{\alpha - \sigma} u \right] \varphi \d x\\
    &= (-1)^{|\alpha|} \int_\Omega \sum_{\rho \leq \alpha} \left[ {\beta \choose \rho - \gamma} + {\beta \choose \rho}\right] \DD^\rho \xi \DD^{\alpha - \rho} u \varphi \d x
  \end{align*}
  with the convention ${\beta \choose \tilde\beta} = 0$ if $\beta_i < \tilde \beta_i$ or $\tilde \beta_i < 0$ for some $i \in \{1,\dots,n\}$.
\end{proof}
