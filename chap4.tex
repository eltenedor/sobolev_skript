...
\begin{proof}
  ...
  \item With
    ...
    Moreover, 
      ...
      \intertext{are all open with $V_i \Subset \Omega$ for all $i \in \N_0$ and  $\Omega = \bigcup_{i = 0}^\infty V_i$.
    Defining further}
  ...
  \item Let $u \in \WW^{k,p}(\Omega)$ be arbitrary.
    ...
    \begin{equation}
      \|u_i - \xi_i u \|_{\WW^{k,p}(\Omega)} = \|u_i - \xi_i u\|_{\WW^{k,p}(Z_i)} \leq \frac{\delta}{2^{i + 1}}
    \end{equation}
    ...
    \intertext{Since $U_i \subset U_{i + 1}$ for all $i \in \N$, $U_i \Subset \Omega$, and $\Omega = \bigcup_{i = 1}^\infty U_i$, we conclude by the monotone convergence theorem }
...
Another condition on $\Omega$ is that $\partial\Omega$ is locally the graph of a $\CC^m$-function.
