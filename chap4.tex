...
\begin{thm}
  \label{thm:interiorApproximation}
...
\begin{proof}
  As $f'$ is continuous and bounded and $u$ is measurable, we have $f' \circ u\in \Ell^\infty(\Omega)$ and $(f' \circ u) \nabla u \in \Ell^p(\Omega)$.
  In view of 
  $$
  |f(x)| \leq |f(0)| + M|x|, \quad\text{for all } x \in \R,
  $$
  the assumption $f(0)= 0$ or $|\Omega| < \infty$ implies $f(u) \in \Ell^p(\Omega)$.
  By Theorem \ref{thm:meyersSerrin} there exists $(u_m)_{m \in \N} \subset \CC^\infty \cap \WW^{1,p}(\Omega)$ such that $u_m \to u$ in $\WW^{1,p}(\Omega)$, hence $u_m \to u$ and $(u_m)_{x_i} \to u_{x_i}$ in $\Ell^p(\Omega)$ for all $i \in \{1,\dots,n\}$.
...
\begin{defn}
  ...
  We say that $\Omega$ satisfies the \emph{segment condition} if for any $x \in \partial\Omega$ there exists a neighborhood $\UU_x \subset \R^n$ of $x$ and $0 \neq y_x \in \R^n$ such that $z + t y_x \in \Omega$ for any $z \in \overline\Omega \cap \UU_x$ and any $\varepsilon \in (0,1)$.
\end{defn}
...
\begin{proof}
  ...
  $$
  Since $\varphi$ ha compact support, it is uniformly continuous on $\R^n$.
